...
Each Julia implementation for the steps can be found under `julia/<chapter-number>/<step-number>.lair.jl`
...
The grammar itself is just a dictionary that map grammar rule names to their rules. And have 1 special entry the `1` that will be used as the start rule. We could make a extra `Grammar` pattern but i decided against it. Because we will just pass the grammar along in each match function and only the new `Reference` pattern that hold a grammar name will lookup the respective grammar rule and apply the match function to it. And if we just use the inbuild dictionary we don't must write functions for adding, deleting and modifying grammar rules. Of course we also need to write a new match function that takes a dictionary, the the start rule with the index `i` and then lookup it up and apply the match function to it. For the names i have chosen to use symbols but if your implementation language don't have them just use string but be careful that your possible PEG DSL may collide then with the `Literal` pattern. 

The only thing left for now is to convert the fixed PEG rules so far to the new dynamic grammar logic with is pretty strait forward.